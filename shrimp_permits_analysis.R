...
# total number of noncompliant households (mailing == 2)
...
permit_dat <- left_join(harvest, permits, by = "permit") %>% group_by(permit) %>%
  summarize(pot_days = sum(pot_days), shrimp = sum(shrimp), mailing = unique(mailing))
...
Sdf_gals <- permit_dat %>% filter(mailing == 2 & !is.na(shrimp)) %>% select(shrimp) %>% summarize(sd = var(shrimp)) %>% as.numeric()
